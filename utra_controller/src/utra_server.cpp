...
void print_joints(float *frames, int con) {
  for (size_t j = 0; j < con; j++) {
    ROS_INFO("%f %f %f %f %f %f", frames[j * 6], frames[j * 6 + 1], frames[j * 6 + 2], frames[j * 6 + 3],
             frames[j * 6 + 4], frames[j * 6 + 5]);
  }
}
...
bool mv_servo_joint(utra_msg::Mservojoint::Request &req, utra_msg::Mservojoint::Response &res) {
  ...
  if (axiz == 6) {
    ...
    // {
    //   ROS_INFO("%f %f %f %f %f
    //   %f",req.frames[j*6],req.frames[j*6+1],req.frames[j*6+2],req.frames[j*6+3],frames[j*6+4],req.frames[j*6+5]);
    // }
